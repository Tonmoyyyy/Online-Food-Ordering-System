...
class User extends Model
{
    ...
    {
        if ($status === null) {
            $status = ($role === 'customer') ? 'inactive' : 'active';
        }
        $this->execute(
            'INSERT INTO users (name, email, password, phone, role, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
            'sssssss',
            [$name, $email, $password, $phone, $role, $status, date('Y-m-d H:i:s')]
        );
    ...
    {
        return $this->fetchAll('SELECT id, name, email, password, phone, role, status, created_at FROM users ORDER BY created_at DESC');
    }

    public function updateStatus(int $id, string $status): int
    {
        return $this->execute('UPDATE users SET status = ? WHERE id = ?', 'si', [$status, $id]);
    }
