...
{
    public function create(int $userId, float $total, string $deliveryAddress): int
    {
        $this->execute('INSERT INTO orders (user_id, total_amount, delivery_address, status, created_at) VALUES (?, ?, ?, ?, ?)', 'idsss', [$userId, $total, $deliveryAddress, 'pending', date('Y-m-d H:i:s')]);
    ...
    public function availableAssignments(): array
    {
        return $this->fetchAll(
            'SELECT o.id, o.total_amount, o.delivery_address, o.delivery_status, o.created_at, u.name AS customer FROM orders o JOIN users u ON u.id = o.user_id WHERE o.status = ? AND (o.delivery_agent_id IS NULL OR o.delivery_agent_id = 0) ORDER BY o.created_at DESC',
            's',
            ['preparing']
        );
    }
    ...
    {
        return $this->execute('UPDATE orders SET delivery_agent_id = ?, delivery_status = ? WHERE id = ?', 'iss', [$agentId, 'pending', $orderId]);
    }

    public function unassignAgent(int $orderId, int $agentId): int
    {
        return $this->execute('UPDATE orders SET delivery_agent_id = NULL, delivery_status = ? WHERE id = ? AND delivery_agent_id = ?', 'sis', ['pending', $orderId, $agentId]);
    }
    ...
    public function updateDeliveryStatus(int $orderId, string $deliveryStatus): int
    {
        if ($deliveryStatus === 'delivered') {
            return $this->execute('UPDATE orders SET delivery_status = ?, status = ?, delivered_at = ? WHERE id = ?', 'ssis', [$deliveryStatus, 'delivered', date('Y-m-d H:i:s'), $orderId]);
        }
